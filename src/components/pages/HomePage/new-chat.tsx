...
const NewChat: React.FC = ({ navigation }) => {
  ...
  const typingTimeoutRef = useRef<NodeJS.Timeout | null>(null); // 타이핑 타임아웃을 저장하는 ref
  ...
  useEffect(() => {
    ...
    setMessages(
      messagesDate.map((msg) => ({
        _id: msg.id,
        text: msg.msg,
        createdAt: new Date(msg.date),
        user: {
          _id: msg.from,
          name: msg.from === 0 ? '쿠키' : '나',
        },
      })),
    );
  }, []);
  ...
  const onSend = useCallback((messages = []) => {
    ...
    setSendText(sendText + messages[0].text + '\n');
    // 기존 타이핑 타임아웃 클리어
    ...
    typingTimeoutRef.current = setTimeout(() => {
      sendMessageToServer(); // 1.5초 후 서버로 메시지 전송
    }, 1500);
  }, []);
  ...
  return (
    <SafeAreaView
      ...
      <GiftedChat
        ...
        renderBubble={(props) => {
          return (
            <Bubble
              {...props}
              wrapperStyle={{
                right: { backgroundColor: palette.primary[500] },
                left: { backgroundColor: palette.neutral[100] },
              }}
            />
          );
        }}
        renderAvatar={(props) => {
          return <Avatar {...props} imageStyle={{ left: [] }} />;
        }}
        renderSystemMessage={(props) => {
          return <SystemMessage {...props} textStyle={{ color: 'red' }} />;
        }}
        ...
        textInputProps={{ placeholder: '메시지 입력', marginLeft: rsWidth * 15 }}
        alwaysShowSend
        renderSend={(props) => {
          return (
            <View>
